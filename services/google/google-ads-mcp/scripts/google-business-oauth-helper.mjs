...
for (const [key, value] of Object.entries({ client_id: clientId, redirect_uri: redirectUri, response_type: "code", scope: "https://www.googleapis.com/auth/business.manage", access_type: "offline", prompt: "consent", state })) authUrl.searchParams.set(key, value);
...
const server = createServer(async (req, res) => {
  try {
    ...
    await writeFile(resolve(artifactDir, "google-business-refresh-token.local"), payload.refresh_token, "utf8");
    res.writeHead(200, { "content-type": "text/html; charset=utf-8" });
    res.end("<h1>Business Profile OAuth concluido</h1><p>Token salvo somente no artefato local.</p>");
  } catch (error) {
...
server.listen(8787, "127.0.0.1", () => console.log(authUrl.toString()));
